...
export default {
  ...
  methods: {
    sortAccounts (xhr, aid) {
      if (xhr && xhr.data && xhr.data.accounts) {
        
        // 接全部的帳號
        let catchData = xhr.data.accounts.filter((e) => {
          ...
          for (let i = 0; i < groups.length; i++) {
            if (groups[i].subgroups) {
              for (let j = 0; j < groups[i].subgroups.length; j++) {
                groups[i].subgroups[j].name = '--' + groups[i].subgroups[j].mail + ' : ' + groups[i].subgroups[j].name
