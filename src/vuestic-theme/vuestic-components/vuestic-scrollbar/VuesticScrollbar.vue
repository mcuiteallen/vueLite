  ...
  methods: {
    ...
    calcThumb () {
      ...
      this.thumb.style.top =
        (-currentMT / this.contentHeight) * this.trackHeight + 'px'
    },
    onContentResize () {
      // 不做特殊的UI設定，因旁邊的拉桿都不能拖拉
    ...
    scroll (e) {
      ...
      // }
      // this.setVertical(delta)
      if (e.path[4].classList[1] === 'device-data-dropdown') {
        let dropdownWrapper = document.querySelector('.device-data-dropdown .scrollbar-content')
        let clientHeight = dropdownWrapper.clientHeight
        let scrollHeight = dropdownWrapper.scrollHeight
    ...
    setVertical (delta) {
      ...
      let nextMT = 0
      if (
        this.maxHeight - (currentMT - delta) > this.contentHeight &&
        delta > 0
      ) {
